...
See `reference/fallback-tier-2-3.md` for the complete fallback recipes:
when `tshark` is missing (use `pcap_summary.py` + `bulk_extractor` + YARA
over raw pcap), when Zeek is missing (use `tshark_wide.py` for the seven
legacy cheap-signal queries; beaconing detection from tshark SYN CSV), and
when Suricata is missing (YARA over pcap as IDS substitute, document the
gap in `00_intake.md`).
